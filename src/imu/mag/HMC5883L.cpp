...
void HMC5883L::read()
{
	...
	magVector[0] = ((((int16_t)_buff[0]) << 8) | _buff[1]); // X
	magVector[2] = ((((int16_t)_buff[2]) << 8) | _buff[3]); // Z
	magVector[1] = ((((int16_t)_buff[4]) << 8) | _buff[5]); // Y

	//Serial.println("X: " + String(magVector[0]) + ", Y: " + String(magVector[1])+ ", Z: " + String(magVector[2]));
